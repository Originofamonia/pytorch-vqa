...
class RawVQA(data.Dataset):
    """
    RawVQA dataset, open-ended
    load images from filename instead of saved features
    """
    def __init__(self, questions_path, answers_path, image_features_path, answerable_only=False):
        ...
        self.image_ids = self.prepare_imageid(answers_json)
        ...
        # v
        if 'train' in answers_path:
            self.img_folder_path = f'D:/CSE6363/hw4/data/train2014/COCO_train2014_000000'
        elif 'val' in answers_path:
            self.img_folder_path = f'D:/CSE6363/hw4/data/val2014/COCO_val2014_000000'
        self.img_transform = utils.get_transform(config.image_size, config.central_fraction)
    ...
    def prepare_imageid(self, answers_json):
        image_ids = [item['image_id'] for item in answers_json['annotations']]
        return image_ids
    ...
    def _load_image(self, image_id):
        ...
        feat = dataset[index].astype('float32')
        img_id_len = len(str(image_id))
        if img_id_len<6:
            image_id = '0'*(6-img_id_len)+str(image_id)
        img_path = self.img_folder_path + f'{image_id}.jpg'
        img = Image.open(img_path).convert('RGB')

        if self.img_transform is not None:
            img = self.img_transform(img)
        
        return torch.from_numpy(feat), img, img_path
    ...
    def __getitem__(self, idx):
        ...
        v, img, img_path = self._load_image(image_id)
        # since batches are re-ordered for PackedSequence's, the original question order is lost
        # we return `item` so that the order of (v, q, a) triples can be restored if desired
        # without shuffling in the dataloader, these will be in the order that they appear in the q and a json's.
        return v, q, a, q_len, idx, img, img_path, q_words, a_words
